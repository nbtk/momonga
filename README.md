...
接続に手こずる環境で増やす価値があるのは**join_retries**である。1回あたり約40秒で、回数に比例して延びる。既に見つけたPANを毎回スキャンし直さずに済む。

scan_retriesはスキャン時間を1回目17秒、2回目35秒、3回目69秒と広げ、**そこで頭打ちにする**。4回目以降は69秒のまま繰り返す。`momonga.open()`が失敗を返すまでの上限は次のとおり。

| scan_retries | 3（既定） | 4 | 5 | 6 | 8 | 10 |
|---|---|---|---|---|---|---|
| スキャンの上限 | 2.0分 | 3.2分 | 4.3分 | 5.5分 | 7.8分 | 10.1分 |

どちらも回数に比例するが、`momonga.open()`が失敗を返すまでの時間はそのぶん延びる。長く粘らせたいなら、呼び出し側のループに持たせるほうが待ち時間が見えて制御しやすい。
