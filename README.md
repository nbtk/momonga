...
Momongaが送出する例外すべての基底クラス。原因を問わず「Momonga由来の失敗」をまとめて捕まえたい場合に使う。pyserialやOSの例外も`MomongaIOError`に包まれるので、これを捕まえておけば取りこぼさない。
...
`momonga.xmit_timeout`で指定した秒数のあいだにパケットを送信できなかったときに送出される。`MomongaNeedToReopen`のサブクラスなので、そちらを捕捉しているコードはそのまま動く。
...
## No Handler At All
`reopen_delays`と`join_retries`を設定すると、捕捉すべきものが残らない書き方もできる。
...
`repeat()`で終わる列は尽きないので、`MomongaNeedToReopen`が呼び出し側に届くことはない。ライブラリが待ち時間を延ばしながらセッションを張り直し続ける。残るのは最初の接続の失敗だけで、systemdや`docker run --restart=always`の下ではプロセスが終了コード1で落ちて再起動され、新しいインタプリタと新しいシリアルハンドルでやり直す。

両方を上げているのは、接続に手こずる環境ではスキャンと参加のどちらが詰まるか決まっていないからである。目安は`momonga.open()`が諦めるまでの時間で、参加は1回約40秒、スキャンは17.5秒・34.7秒・69.1秒と広げてそこで頭打ちなので、4回目以降は1回69秒。上の値だと合わせて約15分。繋がらないあいだ、これがそのまま再試行の間隔になる。

プロセスを落としてスーパーバイザに任せる形では、待つのはスーパーバイザではなくここである点に注意する。systemdの`RestartSec`は既定で100msだし、`docker run --restart=always`も失敗直後から再起動を試みる。つまり待ち時間を持たせたいなら、この2つの回数を増やすか、`RestartSec`のように明示的に設定するかのどちらかで、放っておくと落ちた直後に再起動して同じ失敗を繰り返す。

引き換えに、まれな`MomongaResponseNotExpected`（応答ひとつが読めない）でもプロセスが終わる。再起動にはスキャンとセッション確立の時間がかかるので、電波状況によってはその1つだけ捕捉するとよい。

`tests/error_handling_example.py`に、この形を含む3通りが動く形で入っている。
...
スマートメーターが値を持たないとき、ECHONETは数値のかわりに規定のコードを返す（積算電力量は0xFFFFFFFE、瞬時電力は0x7FFFFFFE、瞬時電流は相ごとに0x7FFE）。Momongaはこれらを`None`として返すので、そのまま数値として扱うと2147483646Wのような値になることはない。
...
接続に手こずる環境では、この2つを増やすと`momonga.open()`を呼び直す回数が減る。どちらが詰まるかは環境によるので、片方だけでなく両方を見ること。

選ぶ目安は`momonga.open()`が失敗を返すまでの時間である。参加は1回あたり約40秒で、回数に比例する。スキャンは1回目17.5秒、2回目34.7秒、3回目69.1秒と窓を広げ、**そこで頭打ちにする**（4回目以降は69.1秒のまま）。

| 回数 | 3（既定） | 4 | 5 | 6 | 8 | 10 | 15 |
|---|---|---|---|---|---|---|---|
| スキャンの上限 | 2.0分 | 3.2分 | 4.3分 | 5.5分 | 7.8分 | 10.1分 | 15.8分 |
| 参加の上限 | 2.0分 | 2.7分 | 3.3分 | 4.0分 | 5.3分 | 6.7分 | 10.0分 |

両方の合計が、`momonga.open()`が戻ってこない最大の時間になる。繋がらない状況が続くとき、**この時間がそのまま再試行の間隔になる**。呼び出し側で待ってから`momonga.open()`を呼び直すなら、その待ち時間との合計が1周期である。
